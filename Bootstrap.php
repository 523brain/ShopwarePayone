...
class Shopware_Plugins_Frontend_MoptPaymentPayone_Bootstrap extends Shopware_Components_Plugin_Bootstrap
{
  ...
  public function install()
  {
    ...
    $this->addAttributes();
    $this->createMenu();
    $this->initializeLanguageSnippets();

    return array('success' => true, 'invalidateCache' => array('backend', 'proxy'));
  }

  /**
   * initialize all language snippets, not initialized snippets are automatically added to db on first use
   */
  protected function initializeLanguageSnippets()
  {
    $namespaceErrorMessages = Shopware()->Snippets()->getNamespace('frontend/MoptPaymentPayone/errorMessages');
    $namespacePaymentMessages = Shopware()->Snippets()->getNamespace('frontend/MoptPaymentPayone/payment');
    $namespaceOtherMessages = Shopware()->Snippets()->getNamespace('frontend/MoptPaymentPayone/messages');
    
    //3rd parameter saveSnippet = true
    $namespaceErrorMessages->get('generalErrorMessage', 'Es ist ein Fehler aufgetreten', true);
    //credit card predifined error messages
    $namespaceErrorMessages->get('creditCardCheckerrorMessage', 'Bitte überprüfen Sie die Angaben auf der Karte.', true);
    $namespaceErrorMessages->get('errorMessage1076', 'Ungültiger Kartentyp. Bitte überprüfen Sie die Angaben auf der Karte.', true);
    $namespaceErrorMessages->get('errorMessage1078', 'Ungültige Kartennummer. Bitte überprüfen Sie die Angaben auf der Karte.', true);
    $namespaceErrorMessages->get('errorMessage33', 'Verfallsdatum ungültig. Bitte überprüfen Sie die Angaben auf der Karte.', true);
    //address check messages
    $namespaceErrorMessages->get('addresscheckErrorMessage', 'Bitte überprüfen Sie die Adresse', true);
    $namespaceErrorMessages->get('addresscheckcorrected', 'Bitte überprüfen Sie die Adresse', true);
    //consumer score messages
    $namespaceOtherMessages->get('consumerscoreNoteMessage', 'Es wird eine Bonitätsprüfung durchgeführt.', true);
    $namespaceOtherMessages->get('consumerscoreAgreementMessage', 'Stimmen Sie der Bonitätsprüfung zu?', true);
    //other messages and error messages
    $namespaceErrorMessages->get('bankaccountcheckErrorMessage', 'Bitte versuchen Sie es mit einer anderen Zahlungsart nochmal.', true);
    $namespaceErrorMessages->get('bankaccountcheckblocked', 'Zahlung mit der angegebenen Bankverbindung zur Zeit leider nicht möglich.', true);
    $namespacePaymentMessages->get('creditCardCvcProcessed', 'Kartenprüfziffer wurde verarbeitet', true);
  }
  ...
  public function onValidateStep3(Enlight_Hook_HookArgs $arguments)
  {
    ...
    $paymentData = $moptPayoneMain->getFormHandler()
            ->processPaymentForm($paymentId, $post, $moptPayoneMain->getPaymentHelper());
  ...
  public function sOrder__sSaveOrder__after(Enlight_Event_EventArgs $args)
  {
    ...
    if ($order = Shopware()->Models()->getRepository('Shopware\Models\Order\Order')
